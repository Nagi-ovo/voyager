...
`community-only` も `enhancement` も付いていない未割り当ての Issue では、`/claim` とコメントすると自動的に担当者へ割り当てられます。ボットが割り当てを確認します。いずれかのラベルが付いた Issue は、メンテナが `/approve @ユーザー名` を返して初めて割り当てられます。
