...
import { type WaveDromPopupSettings, useWaveDromPopupSettings } from '../useWaveDromPopupSettings';

function Harness({
  capture,
  writeSyncStorage,
}: {
  capture: (settings: WaveDromPopupSettings) => void;
  writeSyncStorage: (payload: Record<string, unknown>) => Promise<void>;
}) {
  const settings = useWaveDromPopupSettings(writeSyncStorage);
...
describe('useWaveDromPopupSettings', () => {
  let container: HTMLDivElement;
  let root: Root;
  let settings: WaveDromPopupSettings;
